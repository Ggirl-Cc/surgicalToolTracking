...
# Process ALL clip folders under data/CholecSeg8k/video*/video*_*
base_dir = r"data\CholecSeg8k"
clip_folders = glob.glob(os.path.join(base_dir, "video*", "video*_*"))

print(f"Found {len(clip_folders)} clip folders to process")
...
for folder in clip_folders:
    ...
    for mask_path in mask_files:
        ...
        # Use unique filename combining clip folder name + frame id to avoid collisions
        clip_name = os.path.basename(folder)
        unique_name = f"{clip_name}_{frame_id}"
        
        shutil.copy(raw_img_path, f"dataset/images/{unique_name}.png")
        with open(f"dataset/labels/{unique_name}.txt", "w") as f:
            f.write("\n".join(lines))
        
        total_processed += 1

print(f"Total frames processed with labels: {total_processed}")
print(f"Total frames skipped (no tools visible): {total_skipped}")
